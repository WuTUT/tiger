...
public class ConstFold implements ast.Visitor {
  ...
  public void visit(Add e) {
    ...
    if (left instanceof Exp.Num && right instanceof Exp.Num) {
      int l = ((Exp.Num) left).num;
      int r = ((Exp.Num) right).num;

      try {
        int result = Math.addExact(l, r);
        this.exp = new Exp.Num(result, e.lineNum);
        this.ischanged = true;
        return;
      } catch (Exception ex) {
        if (ex instanceof ArithmeticException) {
          // Warning Add overflow
  ...
  public void visit(Lt e) {
    ...
    Exp.T right = this.exp;
    if (left instanceof Exp.Num && right instanceof Exp.Num) {
      int lf = ((Exp.Num) left).num;
      int ri = ((Exp.Num) right).num;
      if (lf < ri)
  ...
  public void visit(Sub e) {
    ...
    if (left instanceof Exp.Num && right instanceof Exp.Num) {
      ...
      try {
        int result = Math.subtractExact(l, r);
        this.exp = new Exp.Num(result, e.lineNum);
        this.ischanged = true;
        return;
      } catch (Exception ex) {
        if (ex instanceof ArithmeticException) {
          // Warning Add overflow
          this.exp = new Exp.Sub(left, right, e.lineNum);
        } else {
          System.out.println("TODO Here");
          System.exit(-1);
        }
  ...
  public void visit(Times e) {
    ...
    if (left instanceof Exp.Num && right instanceof Exp.Num) {
      ...
      try {
        int result = Math.multiplyExact(l, r);
        this.exp = new Exp.Num(result, e.lineNum);
        this.ischanged = true;
        return;
      } catch (Exception ex) {
        if (ex instanceof ArithmeticException) {
          // Warning Add overflow
          this.exp = new Exp.Times(left, right, e.lineNum);
        } else {
          System.out.println("TODO Here");
          System.exit(-1);
        }
  ...
  public void visit(ProgramSingle p) {
    ...
    for (String itemString : control.Control.trace) {
      if (itemString.equals("ast.ConstFold")) {
